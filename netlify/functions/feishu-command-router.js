...
exports.handler = async (event) => {
  ...
  const challenge = handleUrlVerification(payload, process.env);
  if (challenge) {
    return challenge;
  }

  if (!verifyFeishuToken(payload, process.env)) {
  ...
  try {
    await dispatchGitHubWorkflow(process.env, { eventId });
  } catch (error) {
    return jsonResponse(
      {
...
function handleUrlVerification(payload, env) {
...
async function dispatchGitHubWorkflow(env, options = {}) {
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "feishu-command-router-netlify",
